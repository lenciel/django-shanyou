...
(function($){
  ...
  $.fn.buildAjaxFormOptions = function(options){
    ...
    if (this.data("modal_show") && !options.successCallback) {
      $.extend(options, {successCallback: function() {
  ...
  $.fn.enableChosen = function (options) {
    ...
    controls.each(function(){
      ...
      var defaultOptions = {no_results_text: '没有找到数据', search_contains: true};
      if (width) {
  ...
  $.fn.enablePopoverForCrispyForm = function() {
    ...
    controls.each(function () {
      ...
      var $popover = $('<i class="icon-question-sign"></i>');
      $("label[for='" + this.id.substring(5) + "']").after($popover);
      $popover.popover({
        'placement': 'left',
        'trigger': 'hover',
        'content': $(this).text(),
